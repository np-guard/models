...
func TestOps(t *testing.T) {
	ipb1, err := ipblock.NewIPBlockFromCidrOrAddress("1.2.3.0/24")
	require.Nil(t, err)
	require.NotNil(t, ipb1)
	ipb2, err := ipblock.NewIPBlockFromCidrOrAddress("1.2.3.4")
	require.Nil(t, err)
...
func TestConversions(t *testing.T) {
	...
	ipb2, err := ipblock.NewIPBlockFromCidrList(cidrs)
	require.Nil(t, err)
...
func TestDisjointIPBlocks(t *testing.T) {
	...
	ipb, err := ipblock.NewIPBlockFromCidrList([]string{"1.2.3.4/32", "172.0.0.0/8"})
	require.Nil(t, err)
	...
	ipb2, err := ipblock.NewIPBlockFromCidrList([]string{"1.2.3.0/30"})
	require.Nil(t, err)
...
func TestPrefixLength(t *testing.T) {
	ipb, err := ipblock.NewIPBlockFromCidrOrAddress("42.5.2.8/20")
	require.Nil(t, err)
	prefLen, err := ipb.PrefixLength()
	require.Nil(t, err)
	require.Equal(t, int64(20), prefLen)

	ipb, err = ipblock.NewIPBlockFromCidrOrAddress("42.5.2.8")
	require.Nil(t, err)
	prefLen, err = ipb.PrefixLength()
	require.Nil(t, err)
...
func TestBadPath(t *testing.T) {
	...
	_, err = ipblock.NewIPBlockFromCidrList([]string{"1.2.3.4/20", "1.2.3.4/40"})
	require.NotNil(t, err)
