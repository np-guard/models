...
import (
	...
	hypercube "github.com/np-guard/models/pkg/ds"
	"github.com/np-guard/models/pkg/interval"
)

type Hypercube = hypercube.NProduct[*interval.CanonicalSet]

// cube returns a new hypercube.NProduct created from a single input cube
...
func checkContained(t *testing.T, a, b *Hypercube, expected bool) {
	t.Helper()
...
func TestSwapDimensions(t *testing.T) {
	...
	require.True(t, cube(1, 2).Swap(0, 0).Equal(cube(1, 2)))

	require.True(t, cube(1, 2, 3, 4).Swap(0, 1).Equal(cube(3, 4, 1, 2)))
	require.True(t, cube(1, 2, 1, 2).Swap(0, 1).Equal(cube(1, 2, 1, 2)))

	require.True(t, cube(1, 2, 3, 4, 5, 6).Swap(0, 1).Equal(cube(3, 4, 1, 2, 5, 6)))
	require.True(t, cube(1, 2, 3, 4, 5, 6).Swap(1, 2).Equal(cube(1, 2, 5, 6, 3, 4)))
	require.True(t, cube(1, 2, 3, 4, 5, 6).Swap(0, 2).Equal(cube(5, 6, 3, 4, 1, 2)))
	...
	require.Panics(t, func() { cube(1, 2).Swap(0, 1) })
	require.Panics(t, func() { cube(1, 2).Swap(-1, 0) })
}
